#!/usr/bin/env python3
"""Run deterministic sync ops pipeline through audit log validation in one command."""
...
def run_pipeline(
    ...
    if inject_failure_attempt > 0:
        finalization_cmd.extend(["--inject-failure-attempt", str(inject_failure_attempt)])
    ...
    finalization = _load_json(Path("testnet/launch/sync_ops_finalization_pipeline.json"))
    gate = _load_json(Path("testnet/launch/sync_ops_release_gate.json"))
    audit = _load_json(Path("testnet/launch/sync_ops_audit_log.json"))
    validation = _load_json(Path("testnet/launch/sync_ops_audit_log_validation.json"))

    pipeline_success = (
        all(s["success"] for s in stage_results)
        and bool(finalization.get("pipeline_success", False))
        and gate.get("release_decision", "blocked") == "approved"
        and bool(audit.get("audit_complete", False))
        and bool(validation.get("audit_valid", False))
    )

    artifacts = [
        Path("testnet/launch/sync_ops_finalization_pipeline.json"),
        Path("testnet/launch/sync_ops_release_gate.json"),
        Path("testnet/launch/sync_ops_audit_log.json"),
        Path("testnet/launch/sync_ops_audit_log_validation.json"),
    ]
    artifact_manifest = []
    ...
    report = {
        "suite": "v1.0.3-sync-ops-audit-assurance-pipeline",
        ...
        "summary": {
            ...
            "certificate_status": finalization.get("terminal_summary", {}).get("certificate_status", "unknown"),
            "certificate_valid": bool(finalization.get("terminal_summary", {}).get("certificate_valid", False)),
            ...
            "audit_complete": bool(audit.get("audit_complete", False)),
            "artifacts_present": int(audit.get("artifacts_present", 0)),
            "artifacts_expected": int(audit.get("artifacts_expected", 0)),
            "audit_valid": bool(validation.get("audit_valid", False)),
            "audit_checks_passed": int(validation.get("checks_passed", 0)),
            "audit_checks_total": int(validation.get("checks_total", 0)),
        },
    ...
    lines = [
        "# v1.0.3 Sync Ops Audit Assurance Pipeline",
        "",
        ...
        f"- artifacts_present={report['summary']['artifacts_present']}/{report['summary']['artifacts_expected']}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run deterministic sync ops audit assurance pipeline")
    ...
    parser.add_argument("--release-version", default="1.0.2", help="Release version string for audit log")
    parser.add_argument("--output-json", default="testnet/launch/sync_ops_audit_assurance_pipeline.json")
    parser.add_argument("--output-md", default="testnet/launch/sync_ops_audit_assurance_pipeline.md")
    ...
    print(f"Audit Complete: {report['summary']['audit_complete']}")
